...
items.forEach(item => {
  const base64Data = getBase64Image(map[item.num]);
...
<div class="test-section">
  ...
  <div class="screenshot-box">
    ...
    ${base64Data ? `<img class="screenshot-img" src="${base64Data}" alt="Test Case ${item.num} Screenshot Evidence" />` : '<p><em>[Screenshot Image Attached]</em></p>'}
  </div>
</div>
`;
});
...
const outputPath = path.join(__dirname, 'public/Vasanthi_Engineering_QA_Handover_Report_Base64_NoTables_30_Jul_2026.doc');
fs.writeFileSync(outputPath, docContent);
console.log('SUCCESSFULLY GENERATED NOTABLES BASE64 WORD DOCUMENT');
